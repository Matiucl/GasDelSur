...
const NAV_ITEMS: Record<UserRole, NavItem[]> = {
  admin: [
    ...
    { to: '/admin/routes',    icon: 'map',                label: 'Rutas del Día' },
  ...
  driver: [
    ...
    { to: '/driver/routes',   icon: 'map',                label: 'Mi Ruta' },
    { to: '/driver/orders',   icon: 'local_shipping',     label: 'Mis Pedidos' },
    { to: '/driver/profile',  icon: 'person',             label: 'Mi Perfil' },
  ],
  client: [
    { to: '/client',           icon: 'home',              label: 'Inicio' },
    { to: '/client/new-order', icon: 'add_shopping_cart', label: 'Pedir Gas' },
    { to: '/client/orders',    icon: 'inventory_2',       label: 'Mis Pedidos' },
    { to: '/client/tracking',  icon: 'local_shipping',    label: 'Rastrear' },
    ...
    <aside className="hidden md:flex flex-col h-screen sticky top-0 left-0 p-4 gap-2 bg-surface-container-low border-r border-outline-variant w-64 shrink-0">
      ...
      <div className="flex items-center gap-3 mb-4 p-3 bg-surface-container-high rounded-xl">
        ...
        <div className="min-w-0">
          ...
          <p className="text-xs text-on-surface-variant">{ROLE_LABEL[user.role]}</p>
        </div>
      </div>
      ...
      <div className="border-t border-outline-variant pt-2 flex flex-col gap-1">
        ...
        <button
          ...
          className="flex items-center gap-3 text-on-surface-variant hover:text-error px-4 py-2 hover:bg-error-container/20 rounded-lg transition-all text-sm"
        >
