...
import { MOCK_ORDERS, MOCK_CYLINDERS } from '@/lib/mockData'

export function AdminHomePage() {
  const navigate = useNavigate()
  const illegible = MOCK_CYLINDERS.filter((c) => c.needsManualValidation).length
  const active = MOCK_ORDERS.filter((o) =>
    ['En Ruta', 'En Punto de Entrega', 'En Validación', 'Asignado'].includes(o.status)
  ).length
  const delivered = MOCK_ORDERS.filter((o) => o.status === 'Entregado' || o.status === 'Finalizado').length
  ...
  return (
    <div className="p-4 md:p-8 space-y-6 max-w-[1200px] mx-auto">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        {[
          { label: 'Rutas Activas', value: active, icon: 'local_shipping', color: 'text-primary' },
          { label: 'Entregados hoy', value: delivered, icon: 'check_circle', color: 'text-primary' },
          {
          ...
          { label: 'Eficiencia', value: '94%', icon: 'trending_up', color: 'text-primary' },
        ].map((kpi) => (
          <div
            key={kpi.label}
