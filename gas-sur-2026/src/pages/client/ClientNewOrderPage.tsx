...
export function ClientNewOrderPage() {
  ...
  const [address, setAddress] = useState('Avenida Alemania 01245, Temuco')
  const [receiverName, setReceiverName] = useState('Ana Pérez')
  const [receiverPhone, setReceiverPhone] = useState('9876 5432')
  ...
  const handleConfirm = () => {
    navigate('/client/tracking', { state: { ordered: true } })
  }
    ...
    <div className="p-4 md:p-8 space-y-6 max-w-[1200px] mx-auto">
      <div>
        ...
        <p className="text-sm text-on-surface-variant mt-1">
          Hola Ana, selecciona los detalles para tu entrega en Temuco.
        </p>
      </div>

      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
        {/* Left: Product + Location */}
        <div className="lg:col-span-7 flex flex-col gap-6">
          ...
          <div className="bg-surface-container-lowest p-6 rounded-xl border border-outline-variant">
            <h3 className="text-xs font-bold text-primary mb-4 flex items-center gap-2 uppercase tracking-wider">
              <Icon name="propane_tank" size={18} />
            ...
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
              {PRODUCTS.map((product) => (
                ...
                >
                  <Icon
                    name="propane_tank"
                    className={`text-4xl mb-2 ${selectedKg === product.kg ? 'text-primary' : 'text-on-surface-variant'}`}
                  />
                  <span className={`text-sm font-bold ${selectedKg === product.kg ? 'text-primary' : 'text-on-surface'}`}>
                    {product.kg}kg
                  </span>
            ...
            <div className="flex flex-col gap-4">
              <div className="relative">
                <input
                  ...
                  className="w-full h-14 pl-4 pr-12 rounded-lg border border-outline-variant focus:border-primary focus:ring-1 focus:ring-primary outline-none bg-white text-sm"
                />
                <Icon name="my_location" className="absolute right-4 top-1/2 -translate-y-1/2 text-primary" />
              </div>
              ...
              <div className="w-full h-48 rounded-xl overflow-hidden border border-outline-variant">
                <MapView
                  ...
                  zoom={14}
                  markers={[{ lat: -38.7359, lng: -72.5904, label: address }]}
        ...
        <div className="lg:col-span-5 flex flex-col gap-6">
            ...
            <div className="space-y-4">
              <div>
                ...
                <input
                  ...
                  className="w-full h-12 px-4 rounded-lg border border-outline-variant focus:border-primary focus:ring-1 focus:ring-primary outline-none text-sm"
                />
              </div>
              <div>
                <label className="block text-xs font-semibold text-on-surface-variant mb-1">
                ...
                <div className="flex gap-2">
                  <div className="h-12 px-3 border border-outline-variant rounded-lg bg-surface-container flex items-center text-sm font-medium text-on-surface-variant">
                    +56 9
                  </div>
                  <input
                    type="tel"
                    ...
                    className="flex-1 h-12 px-4 rounded-lg border border-outline-variant focus:border-primary focus:ring-1 focus:ring-primary outline-none text-sm"
                  />
                </div>
              </div>
          ...
          <div className="bg-primary-container text-white p-6 rounded-xl shadow-lg">
            <h3 className="text-xs font-bold mb-4 uppercase tracking-wider opacity-80">
              Resumen del Pedido
            </h3>
            <div className="space-y-3 mb-6">
              ...
              <div className="flex justify-between items-center">
                ...
                <span className="text-xs font-bold uppercase text-white/90">Gratis</span>
              </div>
